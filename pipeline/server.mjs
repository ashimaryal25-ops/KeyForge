...
async function apiPrint(req, res) {
  ...
  const gcode = path.join(OUT, `kf_${slugOf(name)}_${style}.gcode`);
...
go.onclick=function(){var orig=go.textContent;go.disabled=true;go.textContent='Forging…';result.textContent='';
 fetch('/api/print',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:nameEl.value,style:style,start:true})})
